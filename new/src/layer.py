...
def batch_normalization(input_data, name, activation_method="relu", is_train=True):
    ...
    """
    output = tf.compat.v1.layers.batch_normalization(inputs=input_data, momentum=0.9, center=True, scale=True, epsilon=1e-5, name=name,
                        reuse=tf.compat.v1.AUTO_REUSE, training=is_train)
...
def diff_layer(input_data, is_diff, is_diff_abs, is_abs_diff, order, direction, name, padding="same"):
    ...
    if order == 0:
        ...
        if is_diff is True:
            output = tf.compat.v1.nn.conv2d(input=input_data,
                                  filter=filter_diff,
                                  strides=[1, 1, 1, 1],
                                  padding=padding)
            # output = tf.nn.conv2d(input=input_data,
